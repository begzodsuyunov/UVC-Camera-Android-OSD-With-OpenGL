...
public class UVCCamera {
	...
	protected static int mCurrentFrameFormat = FRAME_FORMAT_MJPEG;
	...
	protected static String mSupportedSize;
	...
	protected List<com.serenegiant.usb.custom.Size> mSupportedSizeList;
...
//    		mNativePtr = 0;	// nativeDestroyを呼ぶのでここでクリアしちゃダメ
			...
			try {
				...
				for (int i = 0; i < formatsJSON.length(); i++) {
					...
					if (formatJSON.has("subType") && formatJSON.has("frameDescriptors")) {
						...
						if (formatType == mCurrentFrameFormat
								|| formatType == UVC_VS_FORMAT_UNCOMPRESSED) {
							List<com.serenegiant.usb.Format.Descriptor> descriptorList = parseFrameDescriptors(formatJSON, index);
	...
	private List<com.serenegiant.usb.custom.Size> fetchSupportedSizeList(List<com.serenegiant.usb.Format> formatList) {
		List<com.serenegiant.usb.custom.Size> sizeList = new ArrayList<>();
		for (com.serenegiant.usb.Format format : formatList) {
			for (com.serenegiant.usb.Format.Descriptor descriptor : format.frameDescriptors) {
				List<Integer> fpsList = new ArrayList<>();
				for (com.serenegiant.usb.Format.Interval interval : descriptor.intervals) {
					fpsList.add(interval.fps);
				}
				sizeList.add(new com.serenegiant.usb.custom.Size(descriptor.type, descriptor.width, descriptor.height, descriptor.fps, fpsList));
			}
		}
		return sizeList;
	}
	private List<com.serenegiant.usb.Format.Descriptor> parseFrameDescriptors(final JSONObject format, final int index) throws JSONException {
		List<com.serenegiant.usb.Format.Descriptor> descriptorList = new ArrayList<>();
		...
		for (int j = 0; j < frameDescs.length(); j++) {
			...
			List<com.serenegiant.usb.Format.Interval> intervals = new ArrayList<>();
			...
			if (intervalJSONArray.length() > 0) {
				...
				for (int k = 0; k < intervalJSONArray.length(); k++) {
					...
					com.serenegiant.usb.Format.Interval interval = new com.serenegiant.usb.Format.Interval(
			...
			try {
				descriptorList.add(new com.serenegiant.usb.Format.Descriptor(index, formatType, width, height, defaultFps, defaultFrameInterval, intervals));
			} catch (final Exception e) {
				e.printStackTrace();
			}
		}
	...
	public static List<Size> getSupportedSizeList() {
