...
public class MediaVideoEncoder extends MediaEncoder implements IVideoEncoder {
	...
	public MediaVideoEncoder(final MediaMuxerWrapper muxer, final int width, final int height, String cameraName, final MediaEncoderListener listener) {
		super(muxer, listener);
		if (DEBUG) Log.d(TAG, "MediaVideoEncoder: ");
		//mRenderHandler = MyRenderHandler.createHandler(TAG);
        mRenderHandler = MyRenderHandler.createHandler(TAG,width,height, cameraName);
		mWidth = width;
		mHeight = height;
	}
